...
async function main() {
  ...
  server.listen(PORT, HOST, () => {
    ...
    backfill({ db, options: { log } }).catch((err) =>
      log(`startup backfill error: ${err}`),
    );
  });
...
let realProjectsDir: string | null = null;
function isAllowedTranscriptPath(path: string): boolean {
...
async function handleRequest(
  ...
  if (method === "GET" && url === "/stats") {
    ...
    const vecCount = (
      db.prepare("SELECT COUNT(*) as n FROM vec_chunks").get() as { n: number }
    ).n;
  ...
  if (method === "POST" && url === "/backfill") {
    const raw = await readBody(req);
    let opts: z.infer<typeof BackfillBody> = {};
    try {
    ...
    backfill({ db, options: { ...opts, log } }).catch((err) =>
