...
async function main() {
    ...
    tools: [
      {
        ...
        description:
          "Semantically search all past Claude Code sessions on this machine. " +
        ...
        inputSchema: {
          ...
          properties: {
            ...
            project: {
              type: "string",
              description:
                "Filter to a specific project folder name (e.g. -Users-jane-dev-my-app)",
            },
            kind: {
              type: "string",
            ...
            before: {
              type: "string",
              description:
                "ISO timestamp — only return chunks before this time",
            },
            exact: {
              type: "string",
              description:
                "Exact substring that must appear in the chunk text (case-insensitive) — " +
            ...
            order: {
              ...
              description:
                "'relevance' (default) ranks by hybrid match quality; " +
      ...
      },
      {
        ...
        description:
          "Retrieve surrounding conversation context around a point in a past Claude Code session. " +
        ...
        inputSchema: {
          ...
          properties: {
            ...
            uuid: {
              type: "string",
              description:
                "UUID of the anchor chunk from a search_sessions hit",
            },
            timestamp: {
              type: "string",
              description:
                "ISO timestamp anchor — used when uuid is not provided; resolves to the nearest chunk in the session",
            },
            radius: {
              type: "number",
              description:
                "Chunks before and after to return (default 6, max 20)",
              default: 6,
            },
          },
          required: ["sessionId"],
      ...
      },
      {
        name: "index_status",
        description:
          "Check the current state of the scrolls index: how many files and chunks are indexed, " +
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "index_status") {
      ...
      const vecCount = (
        db.prepare("SELECT COUNT(*) as n FROM vec_chunks").get() as {
          n: number;
        }
      ).n;
