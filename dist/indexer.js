...
import { parseLine, projectFromPath } from "./parser.js";
...
function getNextSeq({ db, sessionId, }) {
    const row = db
        .prepare("SELECT MAX(seq) as max_seq FROM chunks WHERE session_id = ?")
        .get(sessionId);
    return (row?.max_seq ?? -1) + 1;
}
function deleteChunksByFile({ db, fileStem, }) {
    // Pre-1.0 rows have source='' but their session_id matched the file name.
    const ids = db
        .prepare("SELECT id FROM chunks WHERE source = ? OR (source = '' AND session_id = ?)")
...
async function runIndexFile({ db, path, }) {
    const existing = db
    ...
    if (lines.length === 0) {
        db.prepare("INSERT OR IGNORE INTO files (path, byte_offset, mtime, last_indexed_at) VALUES (?, ?, ?, ?)").run(path, newOffset, mtime, Date.now());
    ...
    if (allChunks.length === 0) {
        db.prepare("INSERT OR REPLACE INTO files (path, byte_offset, mtime, last_indexed_at) VALUES (?, ?, ?, ?)").run(path, newOffset, mtime, Date.now());
    ...
    const insertChunk = db.prepare("INSERT OR IGNORE INTO chunks (uuid, session_id, project, cwd, ts, kind, text, seq, is_sidechain, source) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    const insertFts = db.prepare("INSERT INTO chunks_fts(rowid, text) VALUES (last_insert_rowid(), ?)");
    ...
    const insertAll = db.transaction(() => {
        ...
        const current = db
            .prepare("SELECT byte_offset FROM files WHERE path = ?")
            .get(path);
        if ((current?.byte_offset ?? 0) !== startOffset)
            return 0;
        let inserted = 0;
        for (let i = 0; i < allChunks.length; i++) {
            const chunk = allChunks[i];
            if (!seqCache.has(chunk.sessionId)) {
                seqCache.set(chunk.sessionId, getNextSeq({ db, sessionId: chunk.sessionId }));
            }
            const seq = seqCache.get(chunk.sessionId);
            seqCache.set(chunk.sessionId, seq + 1);
            const { changes } = insertChunk.run(chunk.uuid, chunk.sessionId, project, chunk.cwd, chunk.ts, chunk.kind, chunk.text, seq, chunk.isSidechain ? 1 : 0, source);
            if (changes === 1) {
